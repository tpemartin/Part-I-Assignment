...
```{r}
pums <- read.csv("https://raw.githubusercontent.com/amberkuofruit/Part-I-Assignment/master/pp.csv")
```
...
```{r}
ggplot(pums, aes(x=educ, y=wklywge))+geom_smooth()
```
#上圖可觀察到教育程度對每週薪資有明顯正相關

#想看看教育程度大小
```{r}
ggplot(pums, aes(x=educ))+geom_histogram()
```
#上圖顯示大多數人教育程度落在高中，其次為大學學歷
...
```
#上面OLS估計可以看出教育年限貝塔係數為27.75，故當教育程度越高則每週實質薪資越高。教育程度每多一年，每週薪資平均增加27.75
...
```
#上表指出，平均而言大學學歷的薪資高過高中學歷者，但是大學學歷的薪資差距會較高中學歷的薪資差距高。
...
```
#模型簡單迴歸後得到的，在其他條件不變的情況下，大學生的週薪資會比高中生的週薪資高38.4%，這一點在「使用Robust SE」進行t檢定時也是顯著的。
...
#一、萬一資料是集中在某一州，則沒有把各州差異考慮進來。有些州平均而言教育程度、財富能力就是比較高，較多人可以上大學。理想的解決辦法是每一州的樣本資料都平均抽取。

#二、沒有把職業類別考慮進來，平均而言，藍領階級者有大學學歷者較少，而白領階級平均都有大學學歷。教育程度影響了未來可能從事的職業類別，而職業類別直接決定了薪資高低。因此沒有把職業類別考慮進來可能使得college係數有所偏估。