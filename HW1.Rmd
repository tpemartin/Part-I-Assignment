...
#X軸為教育程度，Y軸為每週實質薪資，可看出兩者稍微呈現正相（但不明顯）
ggplot(pums,aes(x=educ, y=log(wklywge)))+geom_point()+geom_smooth()
#X軸為教育程度，Y軸為每週實質薪資取log，可看出兩者稍微呈現正相關（比較明顯）
lm(wklywge~educ,pums)
#進行OLS估計後可以看出教育年限的係數為27.75，故當教育程度越高則每週實質薪資越高（正相關）
```
...
```{r}
filter(pums,educ == 12 | educ == 16) -> pums2
pums2
```
...
#作模型簡單回歸後得到：在其他條件不變的情況下，大學生的週薪資會比高中生的週薪資高38.4%，
#使用Robust SE進行檢定也是顯著的
```
...
#p-value=0.2688>0.05，do not reject Ho:齊質變異數
```

10. 針對簡單迴歸模型，討論college係數可能有偏估的原因。[非電腦題，請用文字論述。]

可能會有內生性問題，即學歷也可能會受到薪資的影響
由以下結果可看出學歷與薪資呈現正相關，使用Robust SE進行檢定也是顯著的
此問題可用2SLS解決
```{r}
mode2 <- college~lwklywge
out3 <- lm(mode2,data=pums2)
library(sandwich)
library(lmtest)
coeftest(out3,vcov=vcovHC(out3,type="HC0"))
```