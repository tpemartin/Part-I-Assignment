...
# 模型簡單回歸後得到的，在其他條件不變的情況下，大學生的周薪資會比高中生的周薪資高38.4%，這一點在「使用Robust SE」進行t檢定時也是顯著的
```
...
college變數此虛擬變數只考慮了教育程度，但可能還有其它會影響實質薪資的變數(如果一變數會影響實質薪資，但不一定會產生偏誤)，例如：性別、年齡、工作經驗、成長背景等等，因此只考慮college變數導致偏誤產生。

造成college變數產生偏誤的原因：
a.此模型可能遺漏了重要的解釋變數，被遺漏的重要解釋變數對實質薪資的影響被涵括在殘差項內，且此殘差項也有可能同時還影響了教育程度，如上所述，因此A.3假設並不成立，產生偏誤。
b.此迴歸可能出現內生性問題，解釋變數和被解釋變數之間相互作用，也就是教育和薪資之間會相互影響，兩者之間相互為因果關係。因此，會進而導致教育與殘差項之間產生關連，違反A.3假設。
