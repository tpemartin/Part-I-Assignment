...
#在其他條件不變下，教育年數每增加一年，對於每週實質薪資增加27.75%。
#且因為wklywge的skew=12.02>0，理應有右偏的情況。
pums %>% ggplot(aes(x=wklywge)) +geom_histogram()
pums %>% ggplot(aes(x=log(wklywge))) +geom_histogram()
#從橫軸為log(wklywge)的圖形看出，確實有右偏的情形。
...
#其他條件不變下，有讀大學的平均log週薪資高，但是變異程度比較大。
```
...
```{r}
rg1 <-lm(lwklywge~college,pums2)
summary(rg1)
library(sandwich)
library(lmtest)
coeftest(rg1,vcov=vcovHC(rg1,type='HC0'))
#若顯著水準=0.1%，因為p-value=2.2e-16<<0.001.，所以落在拒絕域，所以college的係數顯著異於0，也就是說college對於lwklywge有顯著影響。
```

8. 考慮迴歸殘差的有異質變異，且此變異可以被college捕捉。請針對簡單迴歸模型進行FGLS估計。
```{r}
library(nlme)
rg2<-gls(lwklywge~college,data=pums2, weights=varIdent(form=~1|college))
summary(rg2)
```
...
1.因為log週薪資不太可能只被college影響，說不定還有一些遺漏變數我們沒有納入迴歸式。
2.log週薪資和college彼此都會影響彼此，即內生性問題。
