...
```{r}
library(ggplot2)
ggplot(data=pums,aes(x=log(pob),y=log(wklywge),color=(educ)))+
  geom_point()
```
...
class(college)
describe(college)
pums2%>%ggplot(aes(x=college,y=wklywge))+
geom_point()
...
llnmoedwl<-lm(lwklywge~college+yob+pob+qob,data=pums2)

pums2%>%ggplot(aes(x=yob,y=lwklywge,color=college))+
geom_point()
pums2%>%ggplot(aes(x=pob,y=lwklywge,color=college))+
geom_point()
pums2%>%ggplot(aes(x=qob,y=lwklywge,color=college))+
geom_point()
```
...
coeftest(lnmodel,vcov=vcovHC(lnmodel,type='HC0'))
```

10. 針對簡單迴歸模型，討論college係數可能有偏估的原因。[非電腦題，請用文字論述。]

## 
可能仍然遺漏了其他的解釋變數，故會導致college係數的偏誤。諸如性別、大學選擇系所、大學排名、所居住的地區．．．．等
當忘記估算重要的解釋變數時，殘差值裡，且這些殘差值都有可能與college有交互影響，這樣就會影響college的準確度，並且違反了Ａ３假設
