...
```{r}
library(readr)
pums <- read.csv("https://raw.githubusercontent.com/jhwei617/Part-I-Assignment/weichinghsuan/pp.csv")
```
...
#上表可知在其他條件不變下，每增加教育年限1年，對於實質每周薪資增加27.75%，此外可得知wklywge的skew=12.02>0，有右偏情況
...
#因為對wklywge取log後在畫圖顯示的結果較為清楚，教育年限與每周薪資呈正相關
```
...
#在其他條件不變下，大學學歷的平均薪資會優於高中學歷，但同時變異數也較高中學歷大。
```
...
```{r}
reg1<-lm(lwklywge~college,pums2)
summary(reg1)
library(sandwich)
library(lmtest)
...
```{r}
library(nlme)
reg2<-gls(lwklywge~college,data=pums2,weights=varIdent(form=~ 1 | college)) 
summary(reg2)
```
...
可能有其他遺漏變數也會影響周薪資，但是沒有納入迴歸式，所以導致有偏誤的情形發生。