...
```{r}
pums <- read.csv("~/Part-I-Assignment/pp.csv")
```
...
```
執行OLS估計後可以看出教育年限係數為27.75，故當教育程度越高則每週實質薪資越高。
```{r}
rmodel<-lm(log(wklywge)~educ,data=pums)
```
 #建立迴規模式lm(Y~X)，並命名為model
```{r}
summary(rmodel) #檢視迴歸統計量

```
```{r}
pums %>% ggplot(aes(x=educ,y=log(wklywge))) + geom_point()
```
...
```
模型簡單回歸后得到的，在其他條件不變的情況下，大學生的周薪資會比高中生的周薪資高38.4%，這一點在「使用Robust SE」進行t檢定
...
可能會因為有些遺漏變數未考慮而有偏誤現象，或者存在內生性問題
