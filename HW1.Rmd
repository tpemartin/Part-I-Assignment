...
```{r}
library(readr)
pums<-read_csv("https://raw.githubusercontent.com/tpemartin/Part-I-Assignment/master/pp.csv")
```
...
```{r}
library(nlme)
out3<-gls(model,data=pums2,weight=varExp(form=~college))
summary(out3)
```
...
從BP檢定可以看出結果是不拒絕虛無假設，所以應該沒有異質性問題，而college係數有偏估可能是因為放入的解釋變數不夠多。