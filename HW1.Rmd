...
```{r}
pums2<-pums %>% filter(educ==12|educ==16)
pums2
```
...
```{r}
lm(lwklywge~ college,pums2)->t2
summary(t2)
```

8. 考慮迴歸殘差的有異質變異，且此變異可以被college捕捉。請針對簡單迴歸模型進行FGLS估計。
```{r}
GLS<-gls(lwklywge~ college,pums2,weights=varExp(form=~college))
summary(GLS)
```
...
：college為受大學教育和受高中教育的薪資差異，但是也有更多其他因素會影響薪資差異，如父母的社經地位、所在地區、工作的職位等，這些重要解釋變數需要被控制，才能更精準測量出大學教育與高中教育對薪資的影響。
