...
```
可以從X軸為教育程度，Y軸為每週實質薪資時看出正相關的感覺。

```{r}
ggplot(pums, aes(x=educ))+geom_histogram()
```
大部分的人都擁有高中學歷，大學學歷的人比較少。

```{r}
library(psych)
psych::describe(pums)
```
每週薪資（wklywge）存在著明顯的右偏。
...
## 大學學歷的周薪資比高中學歷更高，大學學歷的薪資波動範圍也高於高中學歷，有更大的離離散度
```
...
我認為用college來區分教育程度的範圍太廣，應該再更細分是排名較前面的亦或是排名較後面的college。畢竟現在college已經是普遍的教育程度。除此之外，影響薪資的也有可能是家庭背景(例如家族財力雄厚與否)、性別(職場性別歧視影響老闆用人)、年資、交際手腕...等等，都有可能影響薪資高低。
