...
pums2 %>% group_by(educ) %>% summarise(var(wklywge))
#具大學學歷者其不論每週實質薪資的平均值或是離散程度均較只具高中學歷者高。
```

6. 薪資研究多採用取對數轉換後的薪資，請產生取對數後的薪資變數，並取名為lwklywge。[Hint: 使用log()函數]
```{r}
pums2$wklywge%>%log() -> lwklywge
```

7. 請使用簡單迴歸來估計college對lwklywge的影響，並討論其顯著度。
```{r}
model <- lwklywge~pums2$college
out1 <- lm(model,pums2)
summary(out1)
#執行後顯示出具有大學學歷者其每週實質薪資會多出38.4%,而t-value也會顯著的落在拒絕域。
```
