...
## 我們可以從平均週薪發現教育和薪資成正比,大學學歷比高中學歷高了約200美金
## 大學學歷標準差較大我推測為學歷愈高工作的選擇性可能較高,故波動性偏大
```
...
rem1 <- lm(model,data=pums2)

library(sandwich)
library(lmtest)
coeftest(rem1,vcov=vcovHC(rem1,type="HC0"))
```

8. 考慮迴歸殘差的有異質變異，且此變異可以被college捕捉。請針對簡單迴歸模型進行FGLS估計。
```{r}
library(nlme)
pums2$predicted <- predict(rem1)
pums2$residuals <- residuals(rem1)
